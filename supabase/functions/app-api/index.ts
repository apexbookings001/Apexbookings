...
function renderEmail(job: EmailJob) {
  ...
  const ticketPass = job.kind === 'ticket_ready' && actionUrl ? `<div style="margin:22px 0;padding:22px;text-align:center;border:1px solid #00ff8855;border-radius:20px;background:linear-gradient(145deg,#151519,#0b0b0d)"><div style="font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#00ff88;font-weight:700">Approved · Verified digital ticket</div><img src="cid:apex-ticket-qr" alt="Unique ticket QR code" width="190" height="190" style="display:block;width:190px;height:190px;margin:18px auto 12px;padding:12px;background:#fff;border-radius:18px"/><div style="font-size:12px;color:#a1a1aa">Scan this unique code to open the verified ticket record.</div></div>` : ''
