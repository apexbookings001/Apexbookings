...
export const eventService = {
  async list(organizationId: string): Promise<EventRecord[]> { const { data, error } = await requireSupabase().from('events').select('*, seats(count)').eq('organization_id', organizationId).is('deleted_at', null).order('starts_at'); if (error) throw error; return (data ?? []).map(row => mapEvent(row as Record<string, unknown>)) },
  ...
  async remove(id: string): Promise<void> { const { error } = await requireSupabase().from('events').update({ deleted_at: new Date().toISOString() }).eq('id', id); if (error) throw error },
