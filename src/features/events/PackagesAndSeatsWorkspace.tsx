...
export function PackagesAndSeatsWorkspace({
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[.025] p-4">
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-2">
          {activeDraftPackages.map((pkg, index) => {
            ...
            return <button key={pkg.id} type="button" onClick={() => setSelectedPackageId(pkg.id)} className="w-full rounded-2xl border p-4 text-left" style={{ borderColor: isSelected ? `${pkg.accent ?? pkg.color ?? T.emerald}90` : T.border, background: isSelected ? 'rgba(255,255,255,.06)' : T.card }}>
        ...
        {selectedPackage && <div className="space-y-3 rounded-2xl border border-white/10 bg-white/[.025] p-4">
...
function SeatAvailabilityEditor({
  ...
  return <div className="mt-3 rounded-xl border border-white/10 bg-black/20 p-3">
    ...
    <div className="mt-3 flex max-h-64 flex-wrap gap-1.5 overflow-y-auto" role="listbox" aria-label="Generated package seats">
      {seats.map(seat => {
