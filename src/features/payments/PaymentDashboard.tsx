...
function BankTransferRequests() {
  return <ProductionBankTransferRequests show={() => undefined} />
...
export function PaymentDashboard({ show }: { show: (message: string) => void }) {
  const [tab, setTab] = useState<'reviews' | 'transfers'>('reviews')
  const [records, setRecords] = useState(() => paymentReviewStore.list())
  const [filter, setFilter] = useState<PaymentStatus | 'all'>('all')
  const [selected, setSelected] = useState<PaymentReviewRecord | null>(null)
  ...
  return <div className="space-y-5"><div><p className="text-xs font-mono uppercase tracking-widest text-emerald-400">Manual verification</p><h1 className="font-serif text-2xl font-bold text-white">Payments</h1><p className="mt-1 text-sm text-zinc-500">Review submitted payment proofs and Bank Transfer request states.</p></div><div className="flex gap-2 border-b border-white/10"><button onClick={() => setTab('reviews')} className={`border-b-2 px-3 py-2 text-sm ${tab === 'reviews' ? 'border-emerald-400 text-emerald-300' : 'border-transparent text-zinc-500'}`}>Payment Reviews</button><button onClick={() => setTab('transfers')} className={`border-b-2 px-3 py-2 text-sm ${tab === 'transfers' ? 'border-sky-400 text-sky-300' : 'border-transparent text-zinc-500'}`}>Bank Transfer Requests</button></div>{tab === 'transfers' ? <BankTransferRequests /> : <><div className="flex flex-wrap gap-2">{(['all','pending','approved','rejected','needs_more_information'] as const).map(status => <button key={status} onClick={() => setFilter(status)} className={`rounded-xl px-3 py-2 text-xs ${filter === status ? 'bg-emerald-400 text-zinc-950' : 'bg-white/5 text-zinc-300'}`}>{status.replaceAll('_', ' ')}</button>)}</div><div className="overflow-x-auto rounded-2xl border border-white/10"><table className="w-full min-w-[820px] text-left text-xs"><thead className="bg-white/[.03] text-zinc-500"><tr><th className="p-4">Method</th><th className="p-4">Customer</th><th className="p-4">Event / seat</th><th className="p-4">Package</th><th className="p-4">Amount</th><th className="p-4">Status</th><th className="p-4">Date</th></tr></thead><tbody>{visible.map(record => <tr key={record.id} onClick={() => setSelected(record)} className="cursor-pointer border-t border-white/10 text-zinc-200"><td className="p-4">{methodName[record.method]}</td><td className="p-4"><div>{record.customer}</div><div className="mt-1 text-zinc-500">{record.email}</div></td><td className="p-4">{record.eventName} · {record.seatLabel}</td><td className="p-4">{record.packageName}</td><td className="p-4">${record.amount.toLocaleString()}</td><td className="p-4 capitalize">{record.status.replaceAll('_', ' ')}</td><td className="p-4">{new Date(record.createdAt).toLocaleString()}</td></tr>)}</tbody></table></div>{!visible.length && <div className="rounded-2xl border border-white/10 p-12 text-center text-sm text-zinc-500">No payment records match this filter.</div>}{selected && <div className="fixed inset-0 z-[110] grid place-items-center bg-zinc-950/80 p-4 backdrop-blur"><div className="w-full max-w-2xl rounded-3xl border border-white/10 bg-[#111113] p-6 text-white"><div className="flex justify-between"><div><p className="font-mono text-[10px] uppercase tracking-widest text-emerald-400">Payment review</p><h2 className="font-serif text-xl font-bold">{selected.customer}</h2></div><button onClick={() => setSelected(null)} className="text-zinc-400">×</button></div><div className="mt-4 grid grid-cols-2 gap-3 text-xs text-zinc-400"><p>Method: <span className="text-zinc-100">{methodName[selected.method]}</span></p><p>Amount: <span className="text-zinc-100">${selected.amount.toLocaleString()}</span></p><p>Seat: <span className="text-zinc-100">{selected.seatLabel}</span></p><p>Package: <span className="text-zinc-100">{selected.packageName}</span></p></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">{selected.proofUrls.map((proof, index) => <a key={proof} href={proof} target="_blank" rel="noreferrer" className="overflow-hidden rounded-xl border border-white/10"><img src={proof} alt={`Payment proof ${index + 1}`} className="aspect-square w-full object-cover"/></a>)}</div><textarea value={selected.notes} onChange={event => setSelected({ ...selected, notes: event.target.value })} placeholder="Internal notes" className="mt-5 h-24 w-full rounded-xl border border-white/10 bg-white/5 p-3 text-sm text-white outline-none"/><div className="mt-5 grid grid-cols-3 gap-2"><button onClick={() => update('approved')} className="rounded-xl bg-emerald-400 px-3 py-2.5 text-xs font-bold text-zinc-950">Approve</button><button onClick={() => update('rejected')} className="rounded-xl bg-red-500/15 px-3 py-2.5 text-xs text-red-200">Reject</button><button onClick={() => update('needs_more_information')} className="rounded-xl bg-white/5 px-3 py-2.5 text-xs text-zinc-200">Request upload</button></div></div></div>}</>}</div>
}
